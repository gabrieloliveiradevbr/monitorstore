...
<main class="conteudo-principal cart-page">
    ...
    @if(session('mensagem'))
        <div style="background-color:#d4edda;color:#155724;border:1px solid #c3e6cb;padding:14px 20px;border-radius:8px;margin-bottom:20px;display:flex;align-items:center;gap:10px;font-weight:500;">
    ...
    @if($totalItens > 0)
    <div style="background:#fff;border-radius:10px;padding:16px 20px;margin-bottom:20px;box-shadow:0 2px 12px rgba(0,0,0,.06);">
        ...
        <div style="background:#e9ecef;border-radius:20px;height:8px;overflow:hidden;">
            <div id="progress-frete" style="background:linear-gradient(90deg,#FFD700,#e6c200);height:100%;border-radius:20px;width:{{ $progresso }}%;transition:width .4s;"></div>
        </div>
    </div>
    @endif

    @if(count($carrinho) == 0)
        <div class="empty-cart store-card" style="text-align:center;padding:4rem 2rem;background:#fff;border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,.06);">
            <i class="fa-solid fa-cart-shopping" style="font-size:5rem;color:#d1d5db;margin-bottom:1.5rem;display:block;"></i>
            ...
            <a href="{{ route('loja.monitores') }}" class="btn btn-primary" style="display:inline-block;padding:12px 28px;font-weight:600;border-radius:10px;">
    ...
    @else
        <div class="cart-container">
            ...
            <section class="cart-items-section">
                <form method="POST" action="{{ route('carrinho.atualizar') }}" class="cart-form store-card" id="cartForm">
                    @csrf
                                ...
                                <tr class="cart-row" data-price="{{ $precoUnit }}">
                                    <td data-label="Produto">
                                        <div class="cart-product-info">
                                            <div class="cart-img-wrapper">
                                                <img src="{{ asset('uploads/produtos/' . $item['imagem']) }}" alt="{{ $item['nome'] }}">
                                            </div>
                                            <div class="cart-product-details">
                                                <a href="{{ route('loja.produto', $id) }}" class="cart-product-title">{{ $item['nome'] }}</a>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td data-label="Quantidade">
                                        ...
                                        <div class="qty-control">
                                            ...
                                            <input type="number" name="quantidade[]" value="{{ $item['quantidade'] }}" min="1" class="cart-qty-input" aria-label="Quantidade" onchange="recalcularTotais()">
                                            <button type="button" class="qty-btn" onclick="alterarQtd(this, 1)" aria-label="Aumentar">+</button>
                                        </div>
                                    </td>
                                    <td data-label="Subtotal">
                                        <span class="cart-subtotal">R$ {{ number_format($precoUnit * $item['quantidade'], 2, ',', '.') }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ route('carrinho.remover', $id) }}" class="btn-remove" onclick="confirmarRemocao(event, this)" aria-label="Remover produto">
            ...
            <aside class="cart-summary-section">
                <div class="store-card summary-card">
                    ...
                    <ul class="summary-totals">
                        <li>
                            <span>Subtotal <em class="item-count" id="summary-count">({{ $totalItens }} {{ $totalItens == 1 ? 'item' : 'itens' }})</em></span>
                    ...
                    <div class="summary-actions">
                        <a href="{{ route('carrinho.finalizar') }}" class="btn btn-primary btn-checkout">
...
function recalcularTotais() {
    ...
    document.getElementById('summary-count').textContent = '(' + totalItens + (totalItens === 1 ? ' item)' : ' itens)');
