...
<main class="conteudo-principal">
    ...
    <section class="store-filter-bar">
        <form action="{{ route('loja.perifericos') }}" method="GET" class="filter-form">

            <div class="filter-group">
                <label for="marca">Marca</label>
                <select name="marca" id="marca" class="filter-select">
                    <option value="">Todas as Marcas</option>
                    @foreach(['Logitech','Razer','HyperX','Redragon','Corsair','SteelSeries','Roccat'] as $m)
            ...
            <div class="filter-group">
                ...
                <select name="preco" id="preco" class="filter-select">
                    ...
                    <option value="ate-200"  {{ ($filtros['preco'] ?? '') == 'ate-200'  ? 'selected' : '' }}>Até R$ 200</option>
                    <option value="200-500"  {{ ($filtros['preco'] ?? '') == '200-500'  ? 'selected' : '' }}>R$ 200 a R$ 500</option>
                    <option value="acima-500"{{ ($filtros['preco'] ?? '') == 'acima-500'? 'selected' : '' }}>Acima de R$ 500</option>
                </select>
            </div>
            ...
            <div class="filter-action">
                ...
                @if(array_filter($filtros))
                    <a href="{{ route('loja.perifericos') }}" class="btn-filter-clear" title="Limpar filtros">
    ...
    <section class="store-product-grid">
        ...
        @empty
            ...
            <div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px;">
                ...
                <p style="color:#6b7280; margin-top:0.5rem;">Tente outros filtros ou <a href="{{ route('loja.perifericos') }}">limpe os filtros</a>.</p>
            </div>
